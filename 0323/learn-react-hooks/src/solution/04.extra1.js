...
function Board() {
  ...
  const [squares, setSquares] = React.useState(() => {
    // 지연된 초기화
    // 로컬 스토리지에 키 값이 있는 지 검토
    const valueInStorage = localStorage.getItem(SQUARE_KEY)
    // 키 값이 있다면 ? 키 값을 해석 : 초깃값 반환
    // 초깃화 후에 결과값을 반환
    return valueInStorage ? JSON.parse(valueInStorage) : INITIAL_SQUARES
  })

  const prevKeyRef = React.useRef()

  React.useEffect(() => {
    // 이전 키와 현재 키가 다르면 ? 이전 키를 로컬 스토리지에서 제거
    ...
    // 로컬 스토리지에 키 값을 저장
    localStorage.setItem(SQUARE_KEY, JSON.stringify(squares))
  })
